...
public class Verify {
    ...
    private static List<Constructor<? extends IntSet>> implementations = new ArrayList<Constructor<? extends IntSet>>();
    ...
    public Verify(final String[] args) {
        try {
            implementations.add(new CoarseLockTreap().getClass().getConstructor());
    ...
    private static void exitUsage() {
        System.out.println(
                "Usage:\n" +
                "  java -javaagent:deuceAgent-1.2.0.jar " + Verify.class.getName() + " [options]\n" +
                "Where options are:\n" +
    ...
    public void run() {
        for (Constructor<? extends IntSet> ctor : implementations) {
            for (int n : _threadCounts) {
                ...
                new CorrectnessCheck(n, _keyRange, _readPct, impl,  _timedMillis).run();
            }
        }
    }
